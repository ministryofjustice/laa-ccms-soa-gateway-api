...
public class DocumentServiceTest {
  ...
  @Mock private CoverSheetClient coverSheetClient;
  ...
  public void testUploadDocument(final String caseReference, final String notificationReference) {
    ...
    // Stub the Client to return the mocked response with the dynamic caseReference and
    ...
    // Call the service method
    final ClientTransactionResponse result =
  ...
  public void testDownloadDocument() {
    ...
    // Stub the Client to return the mocked response
    when(documentClient.downloadDocument(soaGatewayUserLoginId, soaGatewayUserRole, documentId))
        .thenReturn(documentElementType);

    // Stub the Mapper to return the mocked gateway response
    when(commonMapper.toDocument(eq(documentElementType))).thenReturn(document);

    // Call the service method
    Document result =
        documentService.downloadDocument(soaGatewayUserLoginId, soaGatewayUserRole, documentId);

    // Verify that the NotificationClient method was called with the expected arguments
    verify(documentClient).downloadDocument(soaGatewayUserLoginId, soaGatewayUserRole, documentId);
  ...
  public void testDownloadDocumentCoverSheet() {
    ...
    // Call the service method
    CoverSheet result =
        documentService.downloadDocumentCoverSheet(
            soaGatewayUserLoginId, soaGatewayUserRole, documentId);

    // Verify that the NotificationClient method was called with the expected arguments
    verify(coverSheetClient)
