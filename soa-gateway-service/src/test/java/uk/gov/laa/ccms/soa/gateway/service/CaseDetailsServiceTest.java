...
public class CaseDetailsServiceTest {
  ...
  void getCaseDetail_returnsMappedCaseDetail() {
    ...
    uk.gov.laa.ccms.soa.gateway.model.CaseDetail expectedDetail =
        new uk.gov.laa.ccms.soa.gateway.model.CaseDetail();

    when(caseServicesClient.getCaseDetail(
            SOA_GATEWAY_USER_LOGIN_ID, SOA_GATEWAY_USER_ROLE, caseReferenceNumber))
        .thenReturn(mockResponse);
    when(caseDetailsMapper.toCaseDetail(mockCase)).thenReturn(expectedDetail);

    // Act
    uk.gov.laa.ccms.soa.gateway.model.CaseDetail result =
