...
public abstract class AbstractSoaService {
  ...
  /**
   * This method calculates the total number of elements from the record count.
   * If the record count or the fetched records are null, it returns the default total elements.
   *
   * @param recordCount The record count object which may contain the number of fetched records.
   * @param defaultTotalElements The default number of total elements to return if the record
   *                             count is null or doesn't contain fetched records.
  ...
  protected int getTotalElementsFromRecordCount(
      ...
      final int defaultTotalElements) {
    return (recordCount != null
        && recordCount.getRecordsFetched() != null)
