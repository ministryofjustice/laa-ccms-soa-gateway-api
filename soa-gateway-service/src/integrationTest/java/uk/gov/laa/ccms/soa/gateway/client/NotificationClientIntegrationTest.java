...
public class NotificationClientIntegrationTest {
  ...
  public void testUpdateNotification_ReturnsData() throws Exception {
    ...
    mockServer
        .expect(
            xpath(
                    "/msg:NotificationUpdateRQ/header:HeaderRQ/header:TransactionRequestID",
                    namespaces)
                .exists())
        .andExpect(
            xpath("/msg:NotificationUpdateRQ/header:HeaderRQ/header:UserLoginID", namespaces)
                .evaluatesTo(testLoginId))
        .andExpect(
            xpath("/msg:NotificationUpdateRQ/header:HeaderRQ/header:UserRole", namespaces)
                .evaluatesTo(testUserType))
        .andExpect(
            xpath("/msg:NotificationUpdateRQ/msg:NotificationID", namespaces)
                .evaluatesTo(notificationId))
        .andExpect(
            xpath("/msg:NotificationUpdateRQ/msg:UserID", namespaces)
                .evaluatesTo(notification.getUserId()))
        .andExpect(
            xpath("/msg:NotificationUpdateRQ/msg:Action", namespaces)
                .evaluatesTo(notification.getAction()))
        .andExpect(
            xpath("/msg:NotificationUpdateRQ/msg:Message", namespaces)
                .evaluatesTo(notification.getMessage()))
        .andRespond(withPayload(notificationUpdateRS_valid));

    NotificationUpdateRS response =
        client.updateNotification(testLoginId, testUserType, notification, notificationId);
